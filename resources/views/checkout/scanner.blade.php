...
@section('title', 'Venue Check-Out — ArihantPLUS')
...
@push('styles')
    <style>
        ...
        .checkout-success .checkout-time {
            font-size: 14px;
            color: var(--muted);
            margin-top: 4px
        }
...
@section('content')
    <div class="scanner-page">
        <div class="scanner-card">
            <h1>📤 Venue Check-Out</h1>
            <p>Scan the same QR code used at check-in to process checkout.</p>
            ...
            <div id="result" class="result-box">
                ...
                <div class="action-btns" id="actions" style="display:none">
                    <button class="btn-checkout" onclick="performCheckout()">✓ Check Out</button>
    ...
    <script>
        ...
        async function validateQr() {
            ...
            try {
                ...
                if (data.valid) {
                    ...
                    // Already checked out — show info but no action button
                    currentCode = code;
                    showAlreadyCheckedOut(data);
                } else {
     ...
     <div class="detail-row"><span class="lbl">Checked In</span><span class="val">${data.checked_in_at}</span></div>
     `;
        ...
        function showAlreadyCheckedOut(data) {
            const box = document.getElementById('result');
            box.className = 'result-box warning';
            box.style.display = 'block';
            document.getElementById('resultTitle').innerHTML = '⚠️ Already Checked Out';
            document.getElementById('actions').style.display = 'none';

            document.getElementById('details').innerHTML = `
     <div class="detail-row"><span class="lbl">Name</span><span class="val">${data.name}</span></div>
     <div class="detail-row"><span class="lbl">Reg #</span><span class="val">${data.registration_number}</span></div>
     <div class="detail-row"><span class="lbl">Checked Out At</span><span class="val">${data.checked_out_at}</span></div>
     `;

            document.getElementById('checkoutResult').innerHTML = `
     <div class="already-out-box">
     <div style="font-size:24px;font-weight:700;color:#ffd700">Already Left</div>
     <div class="out-time">Checked out on ${data.checked_out_at}</div>
     </div>
     `;
        }
        ...
        async function performCheckout() {
            ...
            try {
                const res = await fetch('{{ route("checkout.perform") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
                ...
                if (data.success) {
                    document.getElementById('actions').style.display = 'none';
                    document.getElementById('resultTitle').innerHTML = '✅ Check-Out Complete';
                    document.getElementById('checkoutResult').innerHTML = `
     <div class="checkout-success">
     <div style="font-size:20px;font-weight:700;color:#8ff0b3">${data.name}</div>
     <div class="checkout-time">Checked out at ${data.checked_out_at}</div>
     </div>
     `;
                } else {
